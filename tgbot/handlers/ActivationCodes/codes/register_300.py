...
async def press_button_300(message: Message):
    await message.answer("Введите ID")
    await InputId_300.Id.set()


async def answer_id_300(message: Message, state: FSMContext):
    id = message.text
...
def register_300(dp: Dispatcher):
    dp.register_message_handler(press_button_300, text="300")
    dp.register_message_handler(answer_id_300, state=InputId_300.Id)
